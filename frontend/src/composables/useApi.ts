...
export async function fetchExamTemplate(examId: string): Promise<ExamTemplate> {
  ...
  if (!data.success || !data.data)
    throw new Error(data.error ?? "Prüfung nicht gefunden");
  return data.data;
}
...
export async function startSession(examId: string): Promise<string> {
  ...
  if (!data.success || !data.data)
    throw new Error(data.error ?? "Session-Start fehlgeschlagen");
  return data.data.sessionId;
}

export async function fetchSession(sessionId: string): Promise<ExamSession> {
  const { data } = await api.get<ApiResponse<ExamSession>>(
    `/sessions/${sessionId}`,
  );
  if (!data.success || !data.data)
    throw new Error(data.error ?? "Session nicht gefunden");
  return data.data;
}
...
export async function uploadDiagramImage(
  ...
  const { data } = await api.post<
    ApiResponse<{ answerId: string; imagePath: string }>
  >(`/sessions/${sessionId}/answers/${subtaskId}/upload`, formData, {
...
export async function submitSession(
  ...
  if (!data.success || !data.data)
    throw new Error(data.error ?? "Abgabe fehlgeschlagen");
  return data.data;
}
