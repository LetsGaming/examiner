...
import { db, assembleExam, canAssembleExam, REQUIRED_TASKS, GENERATE_COUNT } from '../db/database.js'
...
async function ensurePoolSize(part: string, apiKey: string): Promise<void> {
  ...
  const current = (db.prepare('SELECT COUNT(*) as cnt FROM tasks WHERE part = ?').get(part) as { cnt: number }).cnt
  ...
  try {
    console.log(`[pool] ${part}: ${current} Tasks — generiere ${genCount} neue...`)
    const newTasks = await generateTasksForPool(part as ExamPart, genCount, apiKey)
...
async function refillPoolInBackground(part: string, apiKey: string): Promise<void> {
  ...
  try {
    ...
    generatingParts.add(part)
    console.log(`[pool-refill] ${part}: ${current}/${target} — generiere ${genCount} Tasks im Hintergrund`)
    const newTasks = await generateTasksForPool(part as ExamPart, genCount, apiKey)
    await insertTasksIntoDB(part, newTasks)
    console.log(`[pool-refill] ${part}: +${newTasks.length} Tasks, Pool jetzt ${current + newTasks.length}`)
  } catch (err) {
...
async function insertTasksIntoDB(part: string, tasks: GeneratedTask[]): Promise<void> {
  ...
  const insertSubtask = db.prepare(`
    INSERT INTO subtasks (id, task_id, label, task_type, question_text,
                          ...
                          expected_elements, mc_options, position)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `)
  ...
  db.transaction(() => {
    for (const task of tasks) {
      ...
      insertTask.run(taskId, part, task.topicArea, task.pointsValue, task.difficulty ?? 'medium')
      for (let i = 0; i < task.subtasks.length; i++) {
        const st = task.subtasks[i]
        insertSubtask.run(
          randomUUID().replace(/-/g, ''), taskId,
          st.label, st.taskType, st.questionText,
...
examRouter.get('/pool-status', (_req, res: Response) => {
  ...
  const status = ['teil_1', 'teil_2', 'teil_3'].map(part => {
    const total = (db.prepare('SELECT COUNT(*) as cnt FROM tasks WHERE part = ?').get(part) as { cnt: number }).cnt
    return { part, total, canAssemble: canAssembleExam(part), sufficient: total >= (POOL_MIN[part] ?? 4) }
  })
  res.json({ success: true, data: { parts: status } })
})

// POST /api/exams/generate
examRouter.post('/generate', generateLimiter, async (req: Request, res: Response) => {
  const apiKey = process.env.OPENAI_API_KEY
  if (!apiKey) return res.status(500).json({ success: false, error: 'OPENAI_API_KEY nicht konfiguriert.' })
  ...
  try {
    ...
    const newTotal = (db.prepare('SELECT COUNT(*) as cnt FROM tasks WHERE part = ?').get(part) as { cnt: number }).cnt
    res.status(201).json({ success: true, data: { added: newTasks.length, poolSize: newTotal } })
  } catch (err) {
...
examRouter.post('/start', async (req: Request, res: Response) => {
  ...
  db.transaction(() => {
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(sessionId, userId, part, title,
      assembled.scenarioName, assembled.scenarioDescription,
    ...
    for (let i = 0; i < assembled.tasks.length; i++) {
      ...
      if (scenario) {
        const subtasks = db.prepare('SELECT id, question_text FROM subtasks WHERE task_id = ?').all(task.id as string) as { id: string; question_text: string }[]
  ...
  if (apiKey) {
    refillPoolInBackground(part, apiKey).catch(() => {/* ignorieren */})
  }
...
function loadSession(sessionId: string) {
  ...
  const tasks = rawTasks.map(t => {
    const subtasks = db.prepare(`
      SELECT id, label, task_type, question_text, points,
             diagram_type, expected_elements, mc_options, position
    ...
    return {
      ...
      subtasks:  subtasks.map(st => ({
        ...
        // FIX: MC-Optionen korrekt zurückgeben
  ...
  const parsedAnswers = answers.map(a => ({
    ...
    evaluation: a.awarded_points != null ? {
...
sessionRouter.put('/:sessionId/answers/:subtaskId', (req: Request, res: Response) => {
  ...
  const existing = db.prepare(
    'SELECT id FROM answers WHERE session_id = ? AND subtask_id = ?'
  ...
  const newAnswer = db.prepare('SELECT id FROM answers WHERE rowid = ?')
...
sessionRouter.post('/:sessionId/answers/:answerId/evaluate', evaluateLimiter, async (req: Request, res: Response) => {
  const apiKey = process.env.OPENAI_API_KEY
  if (!apiKey) return res.status(500).json({ success: false, error: 'OPENAI_API_KEY nicht konfiguriert.' })
  ...
  try {
    ...
    const isDiagram = answer.task_type === 'plantuml' || answer.task_type === 'diagram_upload'

    if (isDiagram) {
      let imageBase64: string | undefined
      let imageMediaType: string | undefined
      if (answer.task_type === 'diagram_upload' && answer.diagram_image_path) {
        ...
        imageMediaType = ext === '.png' ? 'image/png' : ext === '.gif' ? 'image/gif' : 'image/jpeg'
      }
      evaluation = await analyzeDiagram({
        ...
        plantUmlCode:     answer.task_type === 'plantuml' ? (answer.text_value as string) : undefined,
        imageBase64, imageMediaType,
      }, apiKey)
    } else {
      const studentAnswer = answer.task_type === 'mc'
    ...
    `).run(
      req.params.answerId,
      evaluation.awardedPoints, evaluation.maxPoints, evaluation.percentageScore,
      evaluation.ihkGrade, evaluation.feedbackText,
...
sessionRouter.post('/:sessionId/submit', (req: Request, res: Response) => {
  ...
  ).get(req.params.sessionId) as { id: string; max_points: number } | undefined

  if (!session) return res.status(404).json({ success: false, error: 'Aktive Session nicht gefunden.' })
  ...
  const grade   = percent >= 92 ? 'sehr_gut' : percent >= 81 ? 'gut'
    : percent >= 67 ? 'befriedigend' : percent >= 50 ? 'ausreichend'
    : percent >= 30 ? 'mangelhaft' : 'ungenuegend'
