<template>
  ...
  <div class="home-page">
    <header class="home-header">
      ...
      <button class="settings-btn" @click="openSettings" title="KI-Einstellungen">
        ...
        KI-Einstellungen
        <span v-if="aiSettings.keySource === 'user'" class="settings-key-badge">Eigener Key</span>
        <span v-else-if="aiSettings.keySource === 'env'" class="settings-key-badge settings-key-env">Server-Key</span>
        <span v-else class="settings-key-badge settings-key-missing">Kein Key</span>
      </button>
    </header>
      ...
      <div class="home-grid">
        ...
        <div class="home-intro">
          ...
          <div v-else class="ai-indicator ai-indicator-warn">
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
            Kein API-Key — <button class="ai-indicator-link" @click="openSettings">Jetzt einrichten</button>
          </div>
        </div>
    ...
    <div v-if="showSettings" class="modal-backdrop" @click.self="closeSettings">
      <div class="modal" role="dialog" aria-modal="true" aria-label="KI-Einstellungen">
        ...
        <div class="modal-header">
          ...
          <button class="modal-close" @click="closeSettings" aria-label="Schließen">
        ...
        <div class="modal-body">
          ...
          <div class="status-banner" :class="aiSettings.keySource === 'none' ? 'status-warn' : 'status-ok'">
            <template v-if="aiSettings.keySource === 'user'">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
              Eigener API-Key gespeichert
              <span class="status-date" v-if="aiSettings.updatedAt">· gespeichert {{ formatDate(aiSettings.updatedAt) }}</span>
            </template>
            ...
            <template v-else>
              ...
              Kein API-Key konfiguriert — Prüfungen können nicht generiert oder bewertet werden
            </template>
          </div>
          ...
          <div class="field-group">
            ...
            <div class="key-input-row">
              <input
                ...
                class="key-input"
                :placeholder="aiSettings.keySource === 'user' ? '•••••••••••••••••••••• (gespeichert)' : (selectedProviderMeta?.keyHint ?? 'API-Key eingeben…')"
                autocomplete="off"
                spellcheck="false"
              />
              <button class="key-toggle" @click="showKey = !showKey" :title="showKey ? 'Verbergen' : 'Anzeigen'">
                <svg v-if="!showKey" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                <svg v-else width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
              </button>
            </div>
            <p class="field-hint">
              Der Key wird AES-256-verschlüsselt auf dem Server gespeichert. Er verlässt den Server nie —
              er wird ausschließlich für Bewertungsanfragen an {{ selectedProviderMeta?.label ?? 'den gewählten Anbieter' }} verwendet.
            </p>
          </div>
        ...
        <div class="modal-footer">
          ...
          <div class="modal-footer-right">
            <button class="btn-ghost" @click="closeSettings">Abbrechen</button>
...
const selectedProviderMeta = computed<ProviderInfo | null>(
  () => aiSettings.value.providers.find(p => p.id === form.value.provider) ?? null
)

const currentProviderLabel = computed(() =>
  aiSettings.value.providers.find(p => p.id === aiSettings.value.provider)?.label ?? aiSettings.value.provider
)

const currentProviderModel = computed(() =>
  aiSettings.value.providers.find(p => p.id === aiSettings.value.provider)?.textModel ?? ''
)
...
async function saveSettings() {
  ...
  try {
    ...
    settingsError.value = err instanceof Error ? err.message : 'Speichern fehlgeschlagen.'
  } finally {
...
async function deleteKey() {
  ...
  try {
    ...
    settingsError.value = err instanceof Error ? err.message : 'Löschen fehlgeschlagen.'
  } finally {
    settingsSaving.value = false
  }
}
...
.settings-btn:hover { background: rgba(255,255,255,.08); border-color: rgba(255,255,255,.18); color: #e8eaf0; }

.settings-key-badge {
  padding: 2px 8px; border-radius: 20px; font-size: 10px; font-weight: 600;
  background: rgba(34,197,94,.15); color: #86efac; border: 1px solid rgba(34,197,94,.25);
}
.settings-key-env     { background: rgba(99,102,241,.15); color: #a5b4fc; border-color: rgba(99,102,241,.25); }
.settings-key-missing { background: rgba(239,68,68,.12);  color: #fca5a5; border-color: rgba(239,68,68,.2); }
...
.ai-indicator-warn {
  color: #f59e0b; background: rgba(245,158,11,.06); border-color: rgba(245,158,11,.2);
}
...
.ai-indicator-link {
  ...
  color: #f59e0b; font-size: 12px; font-weight: 600; padding: 0; text-decoration: underline;
}
...
.part-pool {
  ...
  padding: 4px 10px; border-radius: 20px; background: rgba(255,255,255,.04);
}
...
.pool-add {
  ...
  padding: 0 2px; line-height: 1; opacity: 0.7; transition: opacity .15s;
}
...
.modal {
  background: #16181f; border: 1px solid rgba(255,255,255,.1);
...
.modal-title {
  ...
  font-size: 15px; font-weight: 700; color: #f0f1f8;
}
.modal-close {
  width: 32px; height: 32px; border-radius: 8px; border: none;
  background: rgba(255,255,255,.06); color: #6b7280; cursor: pointer;
  display: flex; align-items: center; justify-content: center;
  transition: background .15s, color .15s;
}
.modal-close:hover { background: rgba(255,255,255,.1); color: #e8eaf0; }
...
.status-warn { background: rgba(245,158,11,.07); color: #fbbf24; border-color: rgba(245,158,11,.2); }
...
.provider-card:hover { border-color: rgba(255,255,255,.16); background: rgba(255,255,255,.06); }
.provider-active {
  border-color: #6366f1 !important; background: rgba(99,102,241,.12) !important;
}
...
.key-input:focus { border-color: rgba(99,102,241,.6); }
.key-toggle {
  width: 38px; height: 38px; border-radius: 10px; border: 1px solid rgba(255,255,255,.1);
  background: rgba(255,255,255,.04); color: #6b7280; cursor: pointer; flex-shrink: 0;
  display: flex; align-items: center; justify-content: center; transition: background .15s;
}
.key-toggle:hover { background: rgba(255,255,255,.08); color: #e8eaf0; }
...
/* Feedback */
.settings-error   { font-size: 12px; color: #f87171; padding: 8px 12px; border-radius: 8px; background: rgba(239,68,68,.08); border: 1px solid rgba(239,68,68,.2); }
.settings-success { font-size: 12px; color: #86efac; padding: 8px 12px; border-radius: 8px; background: rgba(34,197,94,.08); border: 1px solid rgba(34,197,94,.2); }
...
.btn-ghost:hover { background: rgba(255,255,255,.06); color: #e8eaf0; }
.btn-ghost:disabled { opacity: .4; cursor: not-allowed; }
.btn-danger { color: #f87171; border-color: rgba(239,68,68,.25); }
.btn-danger:hover { background: rgba(239,68,68,.1) !important; color: #fca5a5 !important; }
