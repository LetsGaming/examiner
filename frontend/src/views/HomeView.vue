          ...
          <div class="home-intro">
            ...
            <p class="intro-text">
              <template v-if="specialty === 'fisi'">
                Realitätsnahe IHK AP2-Prüfungen für
                <strong>Fachinformatiker Systemintegration</strong>
                mit KI-generierten Aufgaben zu Netzwerktechnik, Serverinfrastruktur und
                IT-Sicherheit.
              </template>
              <template v-else>
                Realitätsnahe IHK AP2-Prüfungen für
                <strong>Fachinformatiker Anwendungsentwicklung</strong>
            ...
            <div v-for="p in parts" :key="p.value" class="part-row">
              <div class="part-info">
                ...
                <div class="part-details">
                  ...
                  <div class="part-meta">
                    {{ p.duration }} Min · {{ p.points }} Punkte · {{ p.taskCount }} Aufgaben
                  </div>
                </div>
                <div
                  class="part-pool"
                  :class="(pool.status.value[p.value] ?? 0) >= POOL_MIN ? 'pool-ok' : 'pool-low'"
                >
                  <span class="pool-dot" />
              ...
              <p v-if="pool.errors.value[p.value]" class="part-error">
                {{ pool.errors.value[p.value] }}
              </p>
...
.badge-specialty {
  ...
  font-weight: 800;
  letter-spacing: 0.08em; /* purple accent: intentional specialty badge color */
}
.ai-indicator {
  display: flex;
  align-items: center;
...
} /* blue: server AI indicator — static accent */
.warn-fallback {
  color: var(--danger-text);
  background: var(--danger-bg);
  border: 1px solid var(--danger-border);
