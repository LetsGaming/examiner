  ...
  <div class="exam-page">
    ...
    <div class="topbar">
      <div class="topbar-left">
        <button class="topbar-btn" @click="confirmLeave" title="Prüfung beenden">
    ...
    <div class="exam-body">
      ...
      <aside class="sidebar" :class="{ open: navOpen }">
        ...
        <div v-for="(task, ti) in tasks" :key="task.id" class="sidebar-task-group">
        ...
        <div class="exam-main-inner">
          ...
          <div class="subtask-card" v-if="currentSubtask">
            ...
            <div class="sc-header">
              ...
              <span class="sc-type-chip" :class="`chip-${currentSubtask.taskType}`">{{ typeLabel(currentSubtask.taskType) }}</span>
            ...
            <div class="sc-body">
              ...
              <div v-if="currentSubtask.taskType === 'mc'" class="mc-options">
                ...
                <template v-if="currentSubtask.mcOptions && currentSubtask.mcOptions.length > 0">
                  <label
                    v-for="opt in currentSubtask.mcOptions"
                    :key="opt.id"
                    class="mc-opt"
                    :class="{ selected: currentAnswerState?.selectedMcOption === opt.id }"
                  >
                    <input type="radio" :name="`mc-${currentSubtask.id}`" :value="opt.id"
              ...
              <!-- Upload -->
              <div v-else-if="currentSubtask.taskType === 'diagram_upload'" class="upload-area">
                <div class="upload-info">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>
                  Zeichne dein Diagramm auf Papier, fotografiere es und lade es hier hoch.
                </div>
                <div
                  class="upload-zone"
                  :class="{ 'has-file': !!currentAnswerState?.uploadedFile }"
          ...
          <div class="nav-row">
            ...
            <button v-else class="nav-btn nav-submit" @click="confirmSubmit" :disabled="isSubmitting">
    ...
    <Teleport to="body">
      <div v-if="showScenario" class="overlay" @click.self="showScenario = false">
        <div class="overlay-panel">
          ...
          <div class="overlay-body">
            ...
            <p class="overlay-text" v-if="props.scenarioDescription">{{ props.scenarioDescription }}</p>
            <p class="overlay-text" v-else>Alle Aufgaben beziehen sich auf: <strong>{{ props.scenarioName }}</strong></p>
          </div>
        </div>
      </div>
      ...
      <div v-if="showSubmitConfirm" class="overlay" @click.self="showSubmitConfirm = false">
        <div class="confirm-dialog">
          <h3>Prüfung abgeben?</h3>
          <p>{{ answeredCount }} von {{ totalSubtasks }} Unteraufgaben beantwortet.<br>Beantwortete Aufgaben werden von der KI bewertet.</p>
          <div class="confirm-btns">
            <button class="confirm-cancel" @click="showSubmitConfirm = false">Weiterarbeiten</button>
            <button class="confirm-ok" @click="handleSubmit">Jetzt abgeben</button>
          </div>
        </div>
      </div>
      ...
      <div v-if="showLeaveConfirm" class="overlay" @click.self="showLeaveConfirm = false">
        <div class="confirm-dialog">
          ...
          <p>Deine bisherigen Antworten gehen verloren und die Prüfung wird <strong>nicht</strong> bewertet.</p>
          <div class="confirm-btns">
            <button class="confirm-cancel" @click="showLeaveConfirm = false">Weitermachen</button>
            <button class="confirm-ok confirm-ok-danger" @click="handleCancel">Abbrechen &amp; verlassen</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const {
  flatSubtasks, answerStates, answeredCount, isAnswered, getState,
  debouncedSave, cancelPendingSave, persistAll,
} = useAnswerState(props.sessionId, props.tasks)

const { formattedTime, timerClass, startTimer, stopTimer } =
  useExamTimer(props.examPart, () => handleSubmit())
...
function typeLabel(t: string) {
  return ({ freitext: 'Freitext', pseudocode: 'Pseudocode', mc: 'Multiple Choice',
            plantuml: 'PlantUML', diagram_upload: 'Diagramm-Upload' })[t] ?? t
}
function typeShort(t: string) {
  return ({ freitext: 'Text', pseudocode: 'Code', mc: 'MC', plantuml: 'UML', diagram_upload: 'Upload' })[t] ?? t
}
...
function selectMcOption(id: string) {
  const a = currentAnswerState.value; if (!a) return
...
function onFileDrop(e: DragEvent) { const f = e.dataTransfer?.files[0]; if (f) setFile(f) }
...
function restorePreview() {
  const st = currentSubtask.value, a = currentAnswerState.value; if (!st || !a) return
...
function confirmLeave()  { showLeaveConfirm.value = true }
...
.topbar {
  ...
  background: #131620; border-bottom: 1px solid rgba(255,255,255,.07);
  gap: 8px;
}
...
.topbar-btn:hover { background: rgba(255,255,255,.1); color: #e8eaf0; }
...
.topbar-scenario {
  ...
  transition: all .15s; max-width: 300px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
.topbar-scenario:hover { background: rgba(79,70,229,.2); }

.timer {
  display: flex; align-items: center; gap: 5px;
  font-size: 13px; font-weight: 700; font-family: 'SF Mono', monospace;
  color: #d1d5db; padding: 5px 10px; border-radius: 8px;
  background: rgba(255,255,255,.05);
}
.timer-warning  { color: #fbbf24; background: rgba(251,191,36,.1); }
.timer-critical { color: #f87171; background: rgba(248,113,113,.1); animation: blink 1s ease-in-out infinite; }
@keyframes blink { 50% { opacity: .5; } }

/* ─── Progress ─── */
.progress-bar-wrap { height: 2px; background: rgba(255,255,255,.06); flex-shrink: 0; }
.progress-bar-fill { height: 100%; background: #4f46e5; transition: width .4s ease; }
...
.sidebar {
  ...
  background: #131620; border-right: 1px solid rgba(255,255,255,.07);
  padding: 12px 8px; display: flex; flex-direction: column; gap: 1px;
}
.sidebar-top {
  display: flex; justify-content: space-between; align-items: center;
  padding: 4px 8px 10px;
  font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .09em; color: #4b5563;
}
.sidebar-close { background: none; border: none; color: #4b5563; cursor: pointer; font-size: 14px; display: none; }

.sidebar-task-group { margin-bottom: 4px; }
.sidebar-task-title {
...
.stt-num {
  ...
  background: rgba(255,255,255,.08); font-size: 11px; font-weight: 700; color: #6b7280;
  display: flex; align-items: center; justify-content: center; flex-shrink: 0;
}
.stt-name { flex: 1; font-size: 11px; font-weight: 600; color: #9ca3af; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.stt-pts  { font-size: 10px; color: #4b5563; }

.sidebar-sub {
  display: flex; align-items: center; gap: 7px; width: 100%;
  padding: 5px 8px 5px 14px; border-radius: 6px;
  background: none; border: none; cursor: pointer; text-align: left;
  transition: background .12s;
}
.sidebar-sub:hover    { background: rgba(255,255,255,.04); }
.sidebar-sub.sub-active   { background: rgba(79,70,229,.18); }
.sidebar-sub.sub-answered .sub-label { color: #22c55e; }
.sub-label { font-size: 12px; font-weight: 700; color: #6b7280; min-width: 16px; }
.sidebar-sub.sub-active .sub-label { color: #818cf8; }
...
.mobile-nav-btn {
  display: flex; align-items: center; gap: 6px;
  background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.1);
  border-radius: 8px; padding: 7px 12px; font-size: 12px; font-weight: 500; color: #9ca3af; cursor: pointer;
}
...
.th-num  { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .07em; color: #4f46e5; }
.th-sep  { color: #374151; }
...
/* ─── Subtask card ─── */
.subtask-card {
  background: #131620; border: 1px solid rgba(255,255,255,.08);
...
.sc-badge {
  ...
  background: rgba(79,70,229,.3); color: #a5b4fc;
  display: flex; align-items: center; justify-content: center;
...
.chip-freitext       { background: rgba(79,70,229,.2);  color: #a5b4fc; }
.chip-pseudocode     { background: rgba(34,197,94,.15); color: #86efac; }
.chip-mc             { background: rgba(245,158,11,.15);color: #fcd34d; }
.chip-plantuml       { background: rgba(168,85,247,.2); color: #d8b4fe; }
.chip-diagram_upload { background: rgba(249,115,22,.15);color: #fdba74; }
.sc-pts  { margin-left: auto; font-size: 13px; font-weight: 600; color: #6b7280; }

.sc-question {
  padding: 16px; font-size: 14px; line-height: 1.75; color: #d1d5db;
  white-space: pre-wrap; border-bottom: 1px solid rgba(255,255,255,.05);
...
.sc-footer {
  display: flex; align-items: center; gap: 6px;
  padding: 9px 16px; font-size: 11px; color: #374151; font-style: italic;
...
.mc-no-options {
  ...
  padding: 12px 14px; border-radius: 10px; font-size: 13px; color: #f87171;
...
.mc-opt.selected { border-color: #4f46e5; background: rgba(79,70,229,.12); }
...
.code-input {
  flex: 1; min-height: 240px; padding: 12px; border-radius: 10px; resize: vertical; outline: none;
  background: #0a0c12; border: 1.5px solid rgba(255,255,255,.1);
  color: #d1d5db; font-family: 'SF Mono', 'Fira Code', monospace; font-size: 13px; line-height: 1.65;
  transition: border-color .15s;
}
.code-input:focus { border-color: #4f46e5; }
.code-input-full { min-height: 200px; width: 100%; display: block; }
.plantuml-preview {
  min-height: 240px; border-radius: 10px; border: 1.5px solid rgba(255,255,255,.08);
  background: #f8fafc; display: flex; align-items: center; justify-content: center; overflow: hidden;
}
.plantuml-preview img { max-width: 100%; max-height: 300px; object-fit: contain; }
.preview-empty { display: flex; flex-direction: column; align-items: center; gap: 8px; color: #9ca3af; font-size: 12px; }

/* ─── Text ─── */
.text-input {
  width: 100%; min-height: 160px; padding: 12px; border-radius: 10px; resize: vertical; outline: none;
  background: rgba(255,255,255,.04); border: 1.5px solid rgba(255,255,255,.1);
  color: #e8eaf0; font-size: 14px; line-height: 1.7; font-family: inherit;
...
.upload-info {
  display: flex; align-items: center; gap: 7px;
  background: rgba(79,70,229,.1); border-radius: 8px; padding: 9px 12px;
  font-size: 12px; color: #818cf8; margin-bottom: 10px;
}
.upload-zone {
  border: 2px dashed rgba(255,255,255,.12); border-radius: 12px;
  min-height: 160px; display: flex; flex-direction: column;
  align-items: center; justify-content: center; gap: 8px;
  padding: 24px; text-align: center; transition: border-color .2s;
}
...
.upload-hint-text { font-size: 12px; color: #4b5563; }
.upload-btn {
  display: inline-flex; align-items: center; gap: 6px;
  background: #4f46e5; color: white; border: none; border-radius: 8px;
  padding: 9px 18px; font-size: 13px; font-weight: 600; cursor: pointer; margin-top: 4px;
  transition: background .15s;
}
.upload-btn:hover { background: #4338ca; }
.upload-preview-img { max-width: 100%; max-height: 260px; border-radius: 8px; object-fit: contain; }
.upload-replace {
  background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.1);
...
.nav-back   { background: rgba(255,255,255,.06); border-color: rgba(255,255,255,.1); color: #9ca3af; }
.nav-back:hover:not(:disabled) { background: rgba(255,255,255,.1); color: #e8eaf0; }
...
.overlay-panel-wide { max-width: 660px; }
.overlay-header {
  display: flex; justify-content: space-between; align-items: center;
  padding: 14px 18px; border-bottom: 1px solid rgba(255,255,255,.08);
  font-size: 14px; font-weight: 700; color: #f0f1f8; flex-shrink: 0;
}
...
.overlay-scroll { max-height: calc(80dvh - 52px); overflow-y: auto; }
...
/* ─── Belegsatz ─── */
.bs-section { margin-bottom: 24px; padding-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,.07); }
.bs-section:last-child { border-bottom: none; margin-bottom: 0; }
.bs-h1 { font-size: 14px; font-weight: 700; color: #f0f1f8; margin-bottom: 10px; }
.bs-h2 { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .08em; color: #4b5563; margin: 10px 0 6px; }
.bs-table { width: 100%; border-collapse: collapse; }
.bs-table td { padding: 5px 6px; font-size: 12px; line-height: 1.5; border-bottom: 1px solid rgba(255,255,255,.04); vertical-align: top; color: #d1d5db; }
.bs-table tr:last-child td { border-bottom: none; }
.bs-table td:first-child { font-weight: 600; white-space: nowrap; padding-right: 14px; color: #9ca3af; width: 1%; }
.mono { font-family: 'SF Mono', 'Fira Code', monospace !important; font-size: 11.5px !important; }
.sym  { text-align: center; font-size: 14px; color: #818cf8; }
.bs-code {
  background: #0a0c12; border: 1px solid rgba(255,255,255,.08); border-radius: 8px;
  padding: 11px 13px; font-family: 'SF Mono', 'Fira Code', monospace; font-size: 12px;
  line-height: 1.65; white-space: pre; overflow-x: auto; color: #d1d5db;
}
.bs-chips { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 4px; }
.bs-chips span { background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.08); border-radius: 5px; padding: 2px 9px; font-family: monospace; font-size: 12px; color: #9ca3af; }
.bs-link { display: inline-block; margin-top: 8px; font-size: 12px; color: #818cf8; text-decoration: none; }
...
.confirm-cancel {
  flex: 1; padding: 10px; border-radius: 9px; border: 1px solid rgba(255,255,255,.12);
  background: rgba(255,255,255,.06); color: #9ca3af; font-size: 13px; font-weight: 600; cursor: pointer;
}
.confirm-ok {
  flex: 1; padding: 10px; border-radius: 9px; border: none;
  background: #4f46e5; color: white; font-size: 13px; font-weight: 600; cursor: pointer;
}
...
@media (max-width: 768px) {
  .sidebar {
    ...
    transform: translateX(-100%); width: 260px; transition: transform .25s ease;
